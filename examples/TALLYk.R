...
data <- list(dec=dec, stimA=stimA, stimB=stimB, 
             val=val, cues=cues, nCues=nCues, nTrials=nTrials)
...
model {
    for (i in 1:nTrials) {
        out[i,1:2] <- TALLYk(cues[stimA[i],], cues[stimB[i],], s, nCues)
        choice[i] <- out[i,1]
        ev[i] <- out[i,2]
...
samples <- rjags::jags.samples(model, c('gamma', 'ev'), nsamples, thin=nthin)
